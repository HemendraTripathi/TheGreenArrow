...
def get_message(TOKEN,API_TOKEN):
	...
	if not y['ok']:
		...
		for j in range(i):
			...
			try:#get username also
				#normal message
				message_type = y['result'][j]['message']['chat']['type']
				user_name = ""
			except KeyError:
				try:#callback message
					message_type = y['result'][j]['callback_query']['message']['chat']['type']
					user_name = ""
					approval_flag = True
			...
			if message_type == "private" and approval_flag == False:
				try:
					text = y['result'][j]['message']['text']
					if text == "/start_password":#add username for extra security
						chat_id = y['result'][j]['message']['from']['id']
			...
			#-------------------------------------------
				...
				message = y['result'][j]['callback_query']['message']['text']	
				data = y['result'][j]['callback_query']['data']
				order_no = message.split('\n')[1]
				address  = message.split('\n')[2]
				order_no = int(order_no.split(': ')[1])
				callback_id = y['result'][j]['callback_query']['id']
			...
			#-------------------------------
				...
				message = y['result'][j]['callback_query']['message']['text']
				data = callback_id = y['result'][j]['callback_query']['data']
				from_user = y['result'][j]['callback_query']['from']['first_name']
				try:
					...
					force_check = True	
					user_name = from_user
				date = y['result'][j]['callback_query']['message']['date']
				group_name = y['result'][j]['callback_query']['message']['chat']['title']
				chat_id = y['result'][j]['callback_query']['message']['chat']['id']
			...
			#message from group
				...
				try:
					...
					if text.startswith('/') and text != "/start_password":
						...
						user_name = y['result'][j]['message']['from']['first_name']
						chat_id = y['result'][j]['message']['chat']['id']
						

						
						date = y['result'][j]['message']['date']
						group_name = y['result'][j]['message']['chat']['title']
						
						# No need of street file
					#	try:
					#		street = open('streets.json','r')
					#		streets = json.load(street)
					#		temp_streets = streets['streets'][0]
					#		street.close()
					#	except KeyError:
